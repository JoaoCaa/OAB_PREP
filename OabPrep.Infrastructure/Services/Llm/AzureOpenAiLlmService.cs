...
internal sealed class AzureOpenAiLlmService : ILlmService
{
    ...
    public AzureOpenAiLlmService(HttpClient http, AzureOpenAiSettings settings)
    {
        _http = http;
        _settings = settings;
    }
    ...
    public async Task<LlmResponse> SendMessageAsync(LlmRequest request, CancellationToken ct = default)
    {
        ...
        {
            var url = $"{_settings.Endpoint.TrimEnd('/')}/openai/deployments/{_settings.DeploymentName}" +
                      $"/chat/completions?api-version={_settings.ApiVersion}";
            ...
            if (!response.IsSuccessStatusCode)
            {
                ...
                throw new LlmUnavailableException($"Azure OpenAI retornou {(int)response.StatusCode}: {err}");
            }
